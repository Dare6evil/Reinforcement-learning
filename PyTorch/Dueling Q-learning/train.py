...
lr = 0.01
num_episodes = 300
q = modules.DuelingQ(*env.observation_space.shape, env.action_space.n)
optimizer = torch.optim.SGD(q.parameters(), lr)
replay_buffer = data.ReplayBuffer(buffer_size)
state, _ = env.reset()
sync_interval = 20
target_q = modules.DuelingQ(*env.observation_space.shape, env.action_space.n)
...
env.close()
torch.save(q.state_dict(), 'Dueling Q-learning.pth')
