    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            if batch_size < len(replay_buffer):
                ...
                q_batch = q(torch.Tensor(numpy.array(state_batch)).to(device))
                loss = torch.nn.functional.mse_loss((1 - torch.Tensor(numpy.array(done_batch)).to(device)) * gamma *
                                                    q_target(torch.Tensor(numpy.array(next_state_batch)).to(
                                                        device).detach())[
                                                        numpy.arange(batch_size), q_batch.argmax(dim=1)] + torch.Tensor(
                    reward_batch).to(device), q_batch[numpy.arange(batch_size), action_batch])
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            if done:
